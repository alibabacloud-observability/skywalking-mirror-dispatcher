...
| `ARMS_TLS` | 否 | `false` | ARMS 连接是否使用系统 CA 的 TLS |
...
三段 gRPC 链路默认都使用明文。配置 listener 证书和私钥后启用 listener TLS，`OAP_TLS=true`启用 OAP TLS，`ARMS_TLS=true`启用 ARMS TLS。ARMS使用 `8000`、`8090`等明文端口时保持 `ARMS_TLS=false`；只有连接 `443`等TLS endpoint时才设为 `true`。token 不会出现在配置摘要、日志或指标标签中。
