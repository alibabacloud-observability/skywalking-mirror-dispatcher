...
- `internal/logging`：zap 初始化和 lumberjack 轮转；
...
进程日志使用 zap typed field 和生产 JSON encoder，主输出始终是可执行文件同目录的 `skywalking-mirror.log`。`LOG_STDOUT=true`时额外复制到stdout，默认值为 `false`。lumberjack 在文件达到 100 MiB 时轮转，最多保留 5 个备份、最长保留 30 天，并压缩已轮转文件。只记录生命周期和限频失败事件，不为成功 RPC 逐调用写日志。如果可执行文件目录不可写，日志初始化直接失败，避免运维侧误以为文件日志已经生效。

文件 sink 是容量有界的本地运维日志，不承担投递或回放语义。容器部署可以启用可选stdout副本供集中采集；随附Kubernetes清单设置了 `LOG_STDOUT=true`。
...
随附镜像使用固定 digest 的多阶段构建，最终层只有静态链接二进制和非 root distroless runtime。二进制位于 `/app/skywalking-mirror`，`/app` 归 uid/gid `65532` 所有，允许在旁边创建和轮转日志文件。默认 Kubernetes Service 为 `ClusterIP`，只暴露面向 Agent 的 gRPC 端口；Admin 端口保留在 Pod 内供探针和抓取使用。

写入可执行文件同目录意味着随附 Kubernetes 根文件系统不能只读。这是清单的特定约束，不是 Docker 容器的通用限制：普通 `docker run`具有可写容器层。清单仍保留非 root、禁止提权、drop capabilities 和 runtime-default seccomp。轮转文件随 Pod 删除；清单同时启用stdout，由集群日志系统做持久采集。
